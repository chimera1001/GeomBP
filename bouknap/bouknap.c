...
#include <tmwtypes.h>
#include "/usr/local/MATLAB/R2023a/extern/include/mex.h"
#include "/usr/local/MATLAB/R2023a/extern/include/matrix.h"
...
typedef int           mtype;   /* number of each item                 */
...
void merge(allinfo *a, item *h, mtype no)
{
  ...
  /* initialize limits */
  d = palloc(2*a->d.size+1, sizeof(state));
...
void initfirst(allinfo *a, stype ps, stype ws)
{
  ...
  a->d.size  = 1;
  a->d.fset  = palloc(a->d.size+1, sizeof(state)); /* reserve one at end */
  a->d.lset  = a->d.fset;
...
void copyproblem(item *f, item *l, itype *p, itype *w, mtype *m, boolean *x)
{
  register item *i, *n;
...
stype bouknap(itype *w, itype *p, mtype *m, boolean *x, stype c, ntype n)
{
  allinfo a;
  itype y;
  item *tab;
  ...
  inttab  = palloc(SORTSTACK, sizeof(interval));
...
//   itype w[5] = {2,3,4,5,6};
//   mtype m[5] = {2,1,2,1,2};
//   boolean x[5] = {0,0,0,0,0};

//   stype z;
//   z = bouknap(n, &p[0], &w[0], &m[0], &x[0], c);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    itype *wPTR = (itype *) mxGetData(prhs[0]);
    itype *pPTR = (itype *) mxGetData(prhs[1]);
    mtype *mPTR = (mtype *) mxGetData(prhs[2]);

    stype c = (stype) mxGetScalar(prhs[3]);
    ntype n = (ntype) mxGetScalar(prhs[4]);

    // Create x as a MATLAB double vector
    plhs[0] = mxCreateDoubleMatrix(n, 1, mxREAL);
    double *xOUT = mxGetPr(plhs[0]);

    // Allocate internal boolean array for solving
    boolean *xPTR = (boolean *) mxCalloc(n, sizeof(boolean));

    for (i = 0; i < n; ++i) {
        sumTMP += wPTR[i] * mPTR[i];
    }

    if (sumTMP > c) {
        z = bouknap(wPTR, pPTR, mPTR, xPTR, c, n);
    } else {
        for (i = 0; i < n; ++i) {
            xPTR[i] = mPTR[i];
            z += pPTR[i] * mPTR[i];
