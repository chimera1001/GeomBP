...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    stype c = (stype) mxGetScalar(prhs[2]);
    stype lb = (stype) mxGetScalar(prhs[3]);
    stype ub = (stype) mxGetScalar(prhs[4]);
    int def = (int) mxGetScalar(prhs[5]);
    int relx = (int) mxGetScalar(prhs[6]);
    ntype n = (ntype) mxGetScalar(prhs[7]);
    ...
    // Create output x vector (double)
    plhs[0] = mxCreateDoubleMatrix(n, 1, mxREAL);
    double *x = mxGetPr(plhs[0]);

    if (sumTMP > c) {
        z = combo(&tab[0], &tab[n - 1], c, lb, ub, def, relx);
        for (i = 0; i < n; ++i) {
            if (tab[i].x == 1) {
                x[tab[i].id] = 1.0;  // ensure correct index via id
            } else {
                x[tab[i].id] = 0.0;
            }
        }
